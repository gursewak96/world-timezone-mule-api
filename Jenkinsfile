  ...
  stages {
    ...
    steps {
		...
		script {
            ...
            def response = bat(script: 'curl POST -H "Content-Type: application/java" -H "fileName: test1.jks" -H "appName: customer-prc-api"  -H "envName: Sandbox" -H "orgName: Personal" --data-raw "${certDetails}" $url', returnStdout: true)
            echo response
